...
func TestBatchDescribeVolumes(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			executeDescribeVolumesTest(t, cloudInstance, volumeIDs, volumeNames, tc.expErr, tc.volumes)
		})
	}
}
func executeDescribeVolumesTest(t *testing.T, c *cloud, volumeIDs, volumeNames []string, expErr error, volumes []*ec2.Volume) {
	...
	}{
		{
			...
			mockFunc: func(mockEC2 *MockEC2API, ctx context.Context, volumeID, nodeID, path string, dm dm.DeviceManager) {
				...
				gomock.InOrder(
					...
					mockEC2.EXPECT().AttachVolumeWithContext(gomock.Any(), attachRequest).Return(createAttachVolumeOutput(volumeID, nodeID, path, "attached"), nil),
